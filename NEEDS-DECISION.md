...
## G0 — approved 2026-09-03

The user approved G0 with the following baseline interpretations:

1. Use the audited textual chapter/appendix reference baseline. A later audit correction changed the single-digit number pattern to the intended multi-digit pattern; the corrected visible baseline is **200**, exactly matching the plan. The sentinel counts remain `Chapter 5` = 7, `Chapter 8` = 18, `Appendix E` = 13, and `Appendix F` = 13.
2. Use **3,474 visible words** as the Appendix F guard baseline. The plan's later calibration example says approximately 5,580, but the current target file contains 3,474 by the same visible-word method that gives 4,459 for Chapter 5 (close to its stated approximately 4,492).
